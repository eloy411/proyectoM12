class Socket {
    constructor(menu, form, waiting, board, casilla, personaje, pregunta, mensajes, sonido) {
        ...
        popUp() {
            ...
            this.pregunta.renderPista(this.preguntaNueva, this.turn, this.invidencia, this.personaje.numCasilla, this.contadorErrores)
        }
        ...
        escuchas() {
            ...
            this.socket.on('not-conection', ()=>{

                speechSynthesis.cancel()
                this.sonido.renderSound('no hay ninguna sala libre, vamos a crearle una y esperaremos a que alguien se conecte')

                setTimeout(()=>{this.createRoom()},[5000])
            })
            /**--------------------------------------- */

            this.socket.on('not-specific', ()=>{

                speechSynthesis.cancel()
                this.sonido.renderSound('no hay ninguna sala con ese nombre, o no está conectado o no ha escrito bien el nombre, pruebe de nuevo')
            ...
            this.socket.on('denegado', (dato) => {
                ...
                setTimeout(() => { this.mensajes.removeMensajeError() }, [5000])

            })

            /**--------------------------------------- */

            this.socket.on('confirmacion', () => {
    ...
    controlMenu() {
        ...
        this.menu.logout.addEventListener('click',()=>{
        ...
        this.waiting.botonCancelar.addEventListener('click', () => {
            ...
            this.sonido.renderSound('Has cancelado la espera, te enviamos al inicio')

            setTimeout(()=>{this.reloadGame()},[4000])
